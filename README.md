...
`flake.nix` shows the generated top-level inputs and dendritic output shape. From there, read `modules/dendritic.nix`, then `modules/inventory/hosts.nix`, `modules/defaults.nix`, and `modules/inventory/default.nix` to understand how the Den topology is assembled.
